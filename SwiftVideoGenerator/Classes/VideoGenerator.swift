...
let kFailedToStartAssetExportSession = 3
let kMissingVideoURLs = 4
...
public class VideoGenerator: NSObject {
  ...
  public var fileName = "movie"
  
  /// public property to set a multiple type video's background color
  public var videoBackgroundColor: UIColor = UIColor.black
   ...
   */
  public func generate(_ progress: @escaping ((Progress) -> Void), success: @escaping ((URL) -> Void), failure: @escaping ((NSError) -> Void)) {
    
    /// define the input and output size of the video which will be generated by taking the first image's size
    if let firstImage = images.first {
      minSize = firstImage.size
    }
    
    let inputSize = minSize
    let outputSize = minSize
      ...
      if let videoWriter = videoWriter {
        ...
        if videoWriter.startWriting() {
          ...
          videoWriterInput.requestMediaDataWhenReady(on: media_queue, using: { () -> Void in
            ...
            let numImages = self.images.count
            var frameCount = 0
            var elapsedTime: Double = 0
            
            /// calculate the frame duration by dividing the full video duration by the number of images and rounding up the number
            let frameDuration = CMTime(seconds: ceil(Double(self.duration / Double(self.images.count))), preferredTimescale: 1)
            ...
            while (videoWriterInput.isReadyForMoreMediaData && frameCount < numImages) {
              ...
              if self.type == .single {
                ...
                imageForVideo = self.images[frameCount]
                ...
                elapsedTime += self.audioDurations[frameCount] <= 1 ? self.minSingleVideoDuration : self.audioDurations[frameCount]
              }
              ...
              if !self.appendPixelBufferForImage(imageForVideo, pixelBufferAdaptor: pixelBufferAdaptor, presentationTime: nextStartTimeForFrame) {
                error = NSError(
                  ...
                  userInfo: [
                    ...
                    "rawError": videoWriter.error as Any
                  ]
                )
  ...
  public class func mergeMovies(videoURLs: [URL], andFileName fileName: String, success: @escaping ((URL) -> Void), failure: @escaping ((NSError) -> Void)) {
    var error: NSError?
    ...
    guard !_videoURLs.isEmpty else {
      error = NSError(
        domain: kErrorDomain,
        code: kMissingVideoURLs,
        userInfo: ["description": "Missing video paths"]
      )
      ...
      if let documentsPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first {
        ...
        if let exportSession = AVAssetExportSession(asset: composition, presetName: AVAssetExportPresetHighestQuality) {
          ...
          exportSession.exportAsynchronously(completionHandler: {
            ...
            case .failed:
              if let _error = exportSession.error as NSError? {
                failure(_error)
              }
          ...
          error = NSError(
            domain: kErrorDomain,
            code: kFailedToStartAssetExportSession,
            userInfo: ["description": "Can't begin an AVAssetExportSession"]
          )
   ...
   */
  public init?(withImages _images: [UIImage], andAudios _audios: [URL], andType _type: VideoGeneratorType) {
  ...
  private func mergeAudio(withVideoURL videoUrl: URL, success: @escaping ((URL) -> Void), failure: @escaping ((NSError) -> Void)) {
   ...
   */
  fileprivate func appendPixelBufferForImage(_ image: UIImage, pixelBufferAdaptor: AVAssetWriterInputPixelBufferAdaptor, presentationTime: CMTime) -> Bool {
